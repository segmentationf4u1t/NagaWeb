...
export const evaluationsApi = createApi({
  ...
  endpoints: (builder) => ({
    fetchEvaluations: builder.query<ProcessedResults[], void>({
      query: () => ({
        url: 'https://livebench.ai/table_2024_11_25.csv',
      ...
      transformResponse: (csvContent: string) => {
        ...
        const directResults = lines.slice(1)
          .map((line) => {
        ...
        const aliasedResults = Object.entries(modelAdditionalInfo)
          ...
          .map(([modelId, info]) => {
            const sourceResult = directResults.find(
              result => result.model?.toLowerCase() === info.evaluationAlias?.toLowerCase()
            );
        ...
        return [...directResults, ...aliasedResults]
          .sort((a, b) => (b.global_average || 0) - (a.global_average || 0));
      },
    }),
  }),
});
